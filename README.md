...
Transformer 版の走行方策です。すぐにバランスを崩して接地姿勢に落ち込み、そこから復帰できません。参照モーションの走行フェーズは真似できていない状況です。

<img src="assets/strange_run.gif" width="480" alt="引きの視点で見た不自然な走行">

同じ走行タスクで観測された失敗パターンです。両足を交互に使うのではなく、ほぼ片足でのケンケン跳びを獲得してしまっています。影が大きく離れていることからも、不自然な滞空時間で移動していることが分かります。これは、接地などの物理的な正しさよりも「空中で走るポーズを作ってスタイル報酬を騙す」という局所解に最適化が逃げてしまった、AMP特有の弱点が出た例です。
### バックフリップ（失敗）

<img src="assets/backflip_failure.gif" width="560" alt="G1 がバックフリップを試みて着地に失敗する">

**ジャンプと空中での回転の途中までは成立していますが、途中で頭が地面に衝突してしまっています。** 回転の入力自体は学習できている一方、ジャンプ力の弱さおよび、回転タイミングの不一致が原因と考えられます。
