...
AMPを適用していないバージョンです。交互に足を出すのではなく、片足でのケンケン跳びのような動作を獲得してしまいました。不自然な滞空時間を伴って移動していることが確認できます。これは、人間らしい走り方よりも「移動速度報酬」の局所解への最適化を優先してしまった、プレーンな強化学習特有の課題を示す例です。
### バックフリップ（失敗）
